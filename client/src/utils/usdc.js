...
export const getUSDCBalance = async (walletAddress, provider) => {
  ...
  const contract = new web3.eth.Contract(USDC_ABI, USDC);
...
export const approveUSDC = async (provider, value) => {
  ...
  await getBalance(contract, accounts[0]);

  // Todo: replace this with hex string
  const uint256 = 2 ^ (256 - 1);
  await contract.methods
    .approve(contractAddress, uint256)
